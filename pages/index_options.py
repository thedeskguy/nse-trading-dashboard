...
def get_ohlcv(ticker: str) -> pd.DataFrame:
    """Fetch 3-month daily OHLCV with indicators."""
    try:
...
def build_price_chart(df: pd.DataFrame, symbol: str) -> go.Figure:
    """Candlestick + EMA + Bollinger + Volume."""
...
def build_premium_by_expiry_chart(
    ...
    """
    Bar chart of ATM CE and PE premiums across the next N expiries.
    Stars the recommended trade.
    """
    ...
    # Star marker for recommended trade
    ...
    # Star annotation for recommended trade
    if recommended_expiry in shown_expiries:
        idx = shown_expiries.index(recommended_expiry)
...
def build_oi_chart(chain_data: dict, spot: float, symbol: str, expiry: str) -> go.Figure:
    ...
    # If all OI is zero the chart would be blank — return None so caller shows a message
    if df["CE_oi"].sum() == 0 and df["PE_oi"].sum() == 0:
        return None

    # Sort strikes low→high; use strings so Plotly treats them as categories
    df = df.sort_values("strike", ascending=False)  # top = highest strike
    ...
    fig.update_layout(
        ...
        # Plotly 6 parses numeric-string labels as numbers and blows up the range.
        # Pin the range to the categorical index space so bars have proper height.
...
def build_premium_chart(rec: dict, style: str) -> go.Figure:
    """Gauge showing premium target vs stop-loss range."""
...
def render_analysis_summary(rec: dict, symbol: str):
    """Plain-language trade analysis with rationale."""
...
def render_index_tab(label: str, symbol: str):
    ...
    # ── Intraday / Positional recommendation tabs ─────────────────────────────
    ...
    prem_fig = build_premium_by_expiry_chart(
        chain_data=meta,   # use full chain (all expiries) from unfiltered fetch
        spot=spot,
        symbol=symbol,
    ...
    # Use spot from chain_data so ATM window always matches the fetched strikes
...
def main():
    st.set_page_config(
        page_title="Index Options — NIFTY, BANKNIFTY & MIDCPNIFTY",
        page_icon="📊",
        layout="wide",
